...
## 2024-05-15 - Vectorized Mathematical Simplification in Octave/MATLAB
**Learning:** In MATLAB/Octave, intermediate arrays inside vectorized calculations cause significant overhead due to memory allocation and element-wise operations. Algebraic substitution to compute final output directly (avoiding intermediates like CL and CD) yields substantial speedups (~30% for power required calculations).
**Action:** When optimizing MATLAB/Octave code, algebraically condense formulas to minimize the number of intermediate arrays and element-wise operators, preserving the robustness of vector inputs by carefully maintaining element-wise operators (like `.^`).
